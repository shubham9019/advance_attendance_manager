...
class home_login():
    def __init__(self,master):
        ...
        username_label=Label(master,text="Welcome!!!"+self.username,font=("Arial", 36))
        ...
        date_label=Label(master,text="Date",font=("Arial", 22))
        date_label.grid(row=1,column=0)
        status_attendance_label=Label(master,text="Attendance",font=("Arial", 22))
        status_attendance_label.grid(row=1,column=1)
        for i in range(0,len(atte_store)):
            date_label_db=Label(master,text=atte_store[i][2],font=("Arial", 18))
            date_label_db.grid(row=2+i,column=0)
            status_attendance_db=Label(master,text=atte_store[i][1],font=("Arial", 18))
            status_attendance_db.grid(row=2+i,column=1)

        messages_label=Label(master,text="Admin Messages",font=("Arial", 44))
        ...
        date_label_messages=Label(master,text="Date",font=("Arial", 36))
        date_label_messages.grid(row=41,column=0)
        message_content=Label(master,text='Message',font=("Arial", 36))
        message_content.grid(row=41,column=1)
        length_row=len(message_db_store)
        for i in range(0,len(message_db_store)):
            details_messages=Label(master,text=message_db_store[i][0],font=("Arial", 22))
            details_messages.grid(row=42+i,column=1)
            date_message=Label(master,text=message_db_store[i][1],font=("Arial", 22))
            date_message.grid(row=42+i,column=0)

        leave_request_leabel=Label(master,text='Leave Request',font=("Arial", 36))
        leave_request_leabel.grid(row=32,column=1)
        from_date_label=Label(master,text='From Date',font=("Arial", 22))
        from_date_label.grid(row=33,column=0)
        to_date_label=Label(master,text='To Date',font=("Arial", 22))
        to_date_label.grid(row=33,column=1)
        reason_label=Label(master,text='Reason',font=("Arial", 22))
